/**
 * Microsoft Todo OAuth Routes
 *
 * Handles OAuth2 PKCE flow for Microsoft Todo (separate plugin from Outlook)
 * Uses the same Microsoft Identity Platform but different scopes and plugin ID.
 */
...
export async function microsoftTodoOAuthRoutes(
  fastify: FastifyInstance,
  credentialManager: CredentialManager
) {
  /**
   * Initiate OAuth flow for Microsoft Todo using PKCE
   */
  fastify.get('/oauth/microsoft-todo/start', async (_request, reply) => {
  ...
  /**
   * OAuth callback handler for Microsoft Todo (PKCE)
   */
  fastify.get('/oauth/callback/microsoft-todo', async (request, reply) => {
    ...
    try {
      ...
      // Create account record with microsoft-todo plugin ID
      const accountId = await credentialManager.createAccount(
        'com.corelink.microsoft-todo',
        email,
      ...
      return reply.type('text/html').send(`
        ...
        <html>
        <head>
          <title>Microsoft Todo Connected - CoreLink</title>
          <style>
            body {
              ...
              height: 100vh;
              margin: 0;
              background: linear-gradient(135deg, #2564cf 0%, #00bcf2 100%);
            }
            .card {
              background: white;
              padding: 3rem;
        ...
        <body>
          <div class="card">
            ...
            <h1>Microsoft Todo Connected!</h1>
            <p>Your Microsoft Todo account has been successfully connected to CoreLink.</p>
   ...
   */
  fastify.get('/oauth/microsoft-todo/status', async (_request, reply) => {
    try {
      const accounts = await credentialManager.listAccounts('com.corelink.microsoft-todo');
      return reply.send({ accounts });
    } catch (error) {
      fastify.log.error(error, '[MS Todo Status]');
      const hasCredentials = await credentialManager.hasCredentials('com.corelink.microsoft-todo');
   ...
   */
  fastify.delete('/oauth/microsoft-todo', async (_request, reply) => {
    await credentialManager.deleteCredentials('com.corelink.microsoft-todo');
